...
def search_double_low():
    ...
    sql = ''' select date,bond_id,bond_nm,price,increase_rt,premium_rt,year_left,ytm_rt,premium_rt+price as double_low from convert_bond_daily where date = curdate() - 1 and price <> 100 and increase_rt <> 0 order by premium_rt+price limit 30 '''
    data = pd.read_sql(sql, engine)
